...
3. Duplicate the frame for every remaining tweet — repeat the resize-and-fit process each time.
...
This is where a tool built specifically for this conversion — like [Notes2Pic](/app) — replaces steps 2 through 4 entirely:

1. Paste your thread's text.
2. It auto-splits each idea onto its own slide and formats the text to fit, no manual resizing.
3. You review, tweak anything that needs a human eye, and export the full carousel ready to upload.
...
If you've got a thread sitting on your X profile that never made it to Instagram, that's the one to start with — paste it into [Notes2Pic](/app) and see the slides generate in seconds. First carousel's on you to judge; the time it saves is the actual pitch.
